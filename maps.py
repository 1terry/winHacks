...
class maps:
    ...
    def getDistance(self):
        ...
        try:
            ...
            r = requests.get(url + '&origins=' + self.sourceLat + ',' + self.sourceLong +
                             '&destinations=' + self.destLat + ',' + self.destLong + '&key=' + api_key)
            ...
            for key in x:
                if (key == "rows"):
                    ...
                    distance = (x[key][0]['elements'][0]
                                ['distance']['text']).split("km")
            ...
            # Return -1. If the value is negative we know there is an error.
            return (-1)
